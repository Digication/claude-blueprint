...
**Confirm first**: Use **AskUserQuestion** — "This will remove all your personalization and reset Claude Code to defaults. Are you sure?"
- Header: "Remove Profile"
- Options: "Yes, remove it" / "Never mind"
- **If "Never mind"**: Return to existing mode options (re-show Step 0 existing menu)
- **If "Yes, remove it"**: Proceed with cleanup below
...
- Remove `"outputStyle"` from `~/.claude/settings.json`
...
- Tell the user (adapt to tier):
  - **Guided/Supported**: "All done — I've removed your profile. Claude Code is back to its default behavior. If you ever want to set things up again, just tell me and I'll walk you through it."
  - **Standard/Expert**: "Profile removed. Claude Code is back to default. Run `/onboard` anytime to set up again."
...
1. Re-derive output style from the new tier
2. **Check for style conflict**: If the new tier's output style contradicts the preserved communication style (e.g., Expert output style says "don't explain" but communication is "Explain everything"), ask the user:
   - **AskUserQuestion**: "Your communication style is set to '[current style]', but the new comfort level works differently. Want to update your communication style too?"
   - Header: "Communication Style"
   - Options: the same G2 options (Explain everything / Teach as you go / Explain risky things / Be concise)
   - **If user picks a new style**: use it going forward
   - **If user keeps current style**: preserve it (communication section takes precedence over output style defaults)
3. Re-derive safety posture using the new tier + existing purpose (from `.claude/user-context.md`) + style (current or newly updated)
4. Delete the old output style file from `~/.claude/output-styles/` (identify from current `settings.json` `outputStyle` value → map to filename)
5. Regenerate all onboard sections using new tier with updated or preserved purpose/style values (purpose from project file, style from step 2)
6. Jump to Step 5 (Preview and confirm), mode = global-only
...
**For `global-only` mode — sync tier to project file if it exists:**
6a. If `.claude/user-context.md` exists AND the tier has changed, update only the `<!-- onboard:tier -->` section to reflect the new tier. Do not touch the `<!-- onboard:purpose -->` section — purpose is project-scoped and unchanged.
...
<!-- onboard:communication choice="{G2 option label}" -->
...
The `choice` attribute on the communication marker stores the original G2 selection (e.g., `"Explain everything"`, `"Be concise"`). This allows `level-up` and `project-only` modes to read back the exact choice for safety derivation (rule 6 checks for "Be concise") without reverse-mapping from template content.
